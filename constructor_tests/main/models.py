...
class Tests(models.Model):
    ...
    manual = models.TextField(blank=True,
                              verbose_name="Инструкция по выполеннию теста")  # Инструкции по прохождению теста
    number_of_questions = models.PositiveIntegerField(
        verbose_name="Количество вопросов в тесте")  # Количества вопросов в тесте
    number_of_сhoice = models.PositiveIntegerField(
        verbose_name="Количество вариантов ответа")  # Количества вариантов ответа
    time_create = models.DateTimeField(auto_now_add=True)
    time_update = models.DateTimeField(auto_now=True)
    category_key = models.ForeignKey('Categories', on_delete=models.PROTECT, null=True, verbose_name="Категория")
    author = models.ForeignKey(get_user_model(), on_delete=models.SET_NULL, related_name='author_test', null=True,
...
class Result(models.Model):
    user = models.ForeignKey(get_user_model(), on_delete=models.SET_NULL, related_name='user_test', null=True,
                             default=None)
    ...
    the_best_result = models.IntegerField(
        verbose_name="Лучший результата")  # Лучший балл пользователя при прохождении теста
    test_key = models.ForeignKey('Tests', on_delete=models.PROTECT, null=True)
